...
> 1. **Кількісний масштаб (обсяг вимог $|V|$):** коли самих функцій величезна кількість. Навіть при слабкій зв'язності лінійна модель деградує суто через вичерпання робочої пам'яті (*Lost in the Middle*).
> 2. **Топологічна складність (щільність зв'язків $|E|$):** коли вимог небагато, але вони утворюють щільний клубок залежностей. Зв'язок між двома модулями — це завжди стик двох сутностей (формати даних, узгодження протоколів, мережеві затримки, відмовостійкість). Він створює комбінаторний простір невизначеності, тому враховується з підвищеною вагою ($\mu_c > 1$).
...
*   **Приклад 1 (Тривіальна задача, $\Omega < \Omega_{\text{crit}} \approx 15$):**  
    ...
    *Висновок системного аналізу:* **Задача в цілому тривіальна** — тут немає ані критичного обсягу вимог, ані комбінаторного вибуху зв'язків. Накладні витрати на складну мультиагентну систему є надлишковими («стрілянина з гармати по горобцях»). Звичайний лінійний Few-Shot запит впорається швидше, дешевше та бездоганно.
*   **Приклад 2 (Комплексна задача, $\Omega \ge \Omega_{\text{crit}}$):**  
    Платіжний шлюз з підтримкою стандарту безпеки PCI DSS, чергами Kafka та реплікацією PostgreSQL. Тут наявні обидва фактори одночасно: високий масштаб ($|V| = 22$ компоненти) та надзвичайно щільна топологія зв'язків ($|E| = 35$ перехресних обмежень сумісності).  
...
Замість того, щоб просити іншу нейромережу «перевірити, чи безпечна архітектура», ми формалізуємо правила як **топологічні інваріанти архітектурного графа** і перевіряємо їх класичними детермінованими алгоритмами (перевірка досяжності, валідація ланцюгів та типів вузлів).
