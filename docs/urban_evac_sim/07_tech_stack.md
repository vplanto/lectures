...
`NetworkX` занадто повільний для запитів `shortest_path` у реальному часі.

* **Рішення:** Кешування шляхів або використання `igraph` / `graph-tool` (C++ бекенд) для важких розрахунків, якщо сценарій вимагає частого перерахунку маршрутів (Regime C).
