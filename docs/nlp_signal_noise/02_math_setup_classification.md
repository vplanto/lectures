...
- Унікальні токени (IP, timestamps) мають $idf(t, D) = \log \frac{|D|}{1} = \log |D|$ (максимальне значення)
- Але вони не інформативні для класифікації
- Вони "витісняють" важливі слова з високим TF-IDF

**Рішення з нормалізацією:**
- Плейсхолдери `[TIMESTAMP]`, `[IP_ADDRESS]` з'являються в усіх документах
- $idf([TIMESTAMP], D) = \log \frac{|D|}{|D|} = 0$ (мінімальне значення)
- Важливі слова отримують більшу вагу в TF-IDF векторах
...
$$\text{Text} = "Connection refused: database unavailable"$$

$$\text{Tokens} = \{"Connection", "refused", ":", "database", "unavailable"\}$$
...
$$X_{\text{error}} = \{"Connection", "refused", "database", "unavailable"\}$$

$$X_{\text{normal}} = \{"Request", "processed", "successfully"\}$$
...
Для тексту $d$ та словника $V = \{v_1, v_2, \ldots, v_{|V|}\}$ (всі унікальні слова в корпусі), BoW — це вектор:

$$\mathbf{x} = (x_1, x_2, \ldots, x_{|V|})$$

де $x_i$ — кількість разів, що слово $v_i$ з'являється в тексті $d$.

**Приклад:**

Словник: $V = \{"Connection", "refused", "database", "unavailable", "Request", "processed", "successfully"\}$
...
де $D$ — корпус документів, $|D|$ — загальна кількість документів.
...
$$P(c | \mathbf{x}) = P(c | x_1, x_2, \ldots, x_n)$$
...
де:
- $P(c | \mathbf{x})$ — **апостеріорна ймовірність** (що нас цікавить)
- $P(\mathbf{x} | c)$ — **ймовірність спостереження** (likelihood)
...
$$\hat{c} = \arg\max_{c \in C} P(c | \mathbf{x}) = \arg\max_{c \in C} P(\mathbf{x} | c) \cdot P(c)$$
...
- $\mathbf{x} = \{"віагра"\}$ (спрощений приклад з одним словом)

**Теорема Байєса:**

$$P(\text{Spam} | \text{"віагра"}) = \frac{P(\text{"віагра"} | \text{Spam}) \cdot P(\text{Spam})}{P(\text{"віагра"})}$$

**Розрахунок:**

Припустимо:
- $P(\text{Spam}) = 0.1$ (10% листів — спам)
- $P(\text{"віагра"} | \text{Spam}) = 0.8$ (80% спам-листів містять "віагра")
- $P(\text{"віагра"} | \text{Ham}) = 0.01$ (1% нормальних листів містять "віагра")

**Формула повної ймовірності:**

$$P(\text{"віагра"}) = P(\text{"віагра"} | \text{Spam}) \cdot P(\text{Spam}) + P(\text{"віагра"} | \text{Ham}) \cdot P(\text{Ham})$$

$$P(\text{"віагра"}) = 0.8 \cdot 0.1 + 0.01 \cdot 0.9 = 0.08 + 0.009 = 0.089$$

**Результат:**

$$P(\text{Spam} | \text{"віагра"}) = \frac{0.8 \cdot 0.1}{0.089} = \frac{0.08}{0.089} \approx 0.899 = 89.9\%$$
...
$$P(c | w_1, w_2, \ldots, w_n) = \frac{P(c) \prod_{i=1}^{n} P(w_i | c)}{P(w_1, w_2, \ldots, w_n)}$$
...
$$P(\text{"віагра"}, \text{"казино"} | \text{Spam}) = P(\text{"віагра"} | \text{Spam}) \cdot P(\text{"казино"} | \text{Spam})$$

Припустимо:
- $P(\text{"віагра"} | \text{Spam}) = 0.8$
- $P(\text{"казино"} | \text{Spam}) = 0.7$
- $P(\text{Spam}) = 0.1$
...
Для Ham (припустимо $P(\text{"віагра"} | \text{Ham}) = 0.01$, $P(\text{"казино"} | \text{Ham}) = 0.005$):

$$P(\text{Ham} | \text{"віагра"}, \text{"казино"}) \propto 0.9 \times 0.01 \times 0.005 = 0.000045$$

**Нормалізація:**

$$P(\text{Spam} | \text{"віагра"}, \text{"казино"}) = \frac{0.056}{0.056 + 0.000045} \approx 0.9992 = 99.92\%$$
...
2. **Класифікація = Оптимізація:** Задача класифікації зводиться до знаходження $\arg\max P(c | \mathbf{x})$.

3. **Теорема Байєса — Основа:** $P(c | \mathbf{x}) = \frac{P(\mathbf{x} | c) \cdot P(c)}{P(\mathbf{x})}$ дозволяє обчислити апостеріорну ймовірність.

4. **Припущення Незалежності:** Наївне припущення $P(w_1, w_2, \ldots, w_n | c) = \prod P(w_i | c)$ спрощує обчислення, але не завжди вірне.
