---
title: "BERT та Трансформери: Розуміння Контексту в Технічних Логах"
layout: default
...
$$\mathbf{v}_{\text{"bank"}} = (0.2, -0.1, 0.5, \ldots)$$
...
$$\mathbf{v}_{\text{"bank"}}^{(1)} \neq \mathbf{v}_{\text{"bank"}}^{(2)}$$
...
Для BERT:
- $\mathbf{v}_{\text{"Connection"}}^{(1)} \neq \mathbf{v}_{\text{"Connection"}}^{(2)}$
...
де $\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V \in \mathbb{R}^{d \times d_k}$ — матриці ваг.

**Attention Scores:**

$$\text{score}_{ij} = \frac{\mathbf{q}_i \cdot \mathbf{k}_j}{\sqrt{d_k}}$$

де $d_k$ — розмірність Key (зазвичай $d_k = d$).
...
**Key ($\mathbf{K}$): "Що я можу запропонувати?"**
...
**Value ($\mathbf{V}$): "Що я фактично передаю?"**
...
$$\text{score}_{ij} = \frac{\mathbf{q}_i \cdot \mathbf{k}_j}{\sqrt{d_k}} = \frac{\mathbf{q}_i^T \mathbf{k}_j}{\sqrt{d_k}}$$

**Геометрична інтерпретація:**
- $\mathbf{q}_i \cdot \mathbf{k}_j$ — косинус кута між векторами $\mathbf{q}_i$ та $\mathbf{k}_j$ (нормований на довжини)
...
**Attention weights:**
- $\alpha_{\text{refused}, \text{Connection}} \approx 0.4$ (високий — "Connection" важливий для "refused")
- $\alpha_{\text{refused}, \text{database}} \approx 0.3$ (високий — "database" важливий)
- $\alpha_{\text{refused}, \text{by}} \approx 0.1$ (низький — "by" неважливий)

**Результат:**
$$\mathbf{h}_{\text{refused}} = 0.4 \cdot \mathbf{v}_{\text{Connection}} + 0.3 \cdot \mathbf{v}_{\text{database}} + 0.1 \cdot \mathbf{v}_{\text{by}} + \ldots$$
...
- Якщо два токени схожі в оригінальному просторі ($\mathbf{H}$), вони можуть залишитися схожими або стати різними в проекційних просторах залежно від $\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V$
...
1. **Лінійні відображення:** $\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V$ — це матриці лінійних відображень
2. **Власні значення та вектори:** Підпростори можуть бути пов'язані з власними підпросторами цих матриць
3. **Ортогональність:** У деяких архітектурах підпростори Q, K, V намагаються бути ортогональними
4. **Ранг матриці:** Ранг $\mathbf{W}_Q, \mathbf{W}_K, \mathbf{W}_V$ визначає розмірність підпростору ознак
...
**Ідея:** Використовуємо кілька "голів" attention паралельно, кожна з яких фокусується на різних аспектах.
...
**Параметри:**
- $h$ — кількість голів (зазвичай 8-16)
- $d_k = d_v = d / h$ — розмірність кожної голови
...
**Крок 1: Обчислення $\mathbf{Q}\mathbf{K}^T$**

- $\mathbf{Q}$ має розмірність $n \times d_k$
- $\mathbf{K}^T$ має розмірність $d_k \times n$
- Результат: $\mathbf{Q}\mathbf{K}^T \in \mathbb{R}^{n \times n}$

**Складність матричного множення:** $O(n \times d_k \times n) = O(n^2 d_k)$

Оскільки $d_k$ — константа (наприклад, 64 для BERT-base), складність: **$O(n^2)$**

**Крок 2: Softmax над матрицею $n \times n$**

- Softmax обчислюється для кожного рядка матриці $\mathbf{Q}\mathbf{K}^T$
...
**Складність матричного множення:** $O(n \times n \times d_k) = O(n^2 d_k) = O(n^2)$
...
Оскільки голови обчислюються паралельно (або послідовно, але незалежно), складність залишається:

$$\text{Multi-Head Attention: } O(h \cdot n^2) = O(n^2)$$

(Оскільки $h$ — константа, наприклад, 12 для BERT-base)
...
**Завдання 1:** Обчисліть складність для послідовності довжиною $n = 1024$ токенів.

**Рішення:**

1. **Обчислення $\mathbf{Q}\mathbf{K}^T$:**
   - Операцій: $n \times d_k \times n = 1024 \times 64 \times 1024 = 67,108,864$
   - Складність: $O(1024^2) = O(1,048,576)$

2. **Softmax:**
   - Для кожного з $n$ рядків: $n$ експонент + $n$ ділень
   - Операцій: $n \times 2n = 2n^2 = 2 \times 1,048,576 = 2,097,152$
   - Складність: $O(n^2)$

3. **Множення на $\mathbf{V}$:**
   - Операцій: $n \times n \times d_k = 1024 \times 1024 \times 64 = 67,108,864$
   - Складність: $O(n^2)$

**Загальна складність:** $O(n^2) = O(1,048,576)$ операцій для одного attention head.

**Для Multi-Head з $h = 12$:** $O(12 \times n^2) = O(12,582,912)$ операцій.
...
$$\mathbf{h}_{[CLS]} = \text{BERT}(\text{"Connection refused by database"})[0]$$

де індекс $[0]$ позначає позицію `[CLS]`.

**Класифікація:**

$$\mathbf{y} = \text{softmax}(\mathbf{W} \mathbf{h}_{[CLS]} + \mathbf{b})$$

де $\mathbf{W} \in \mathbb{R}^{C \times d}$, $C$ — кількість класів.
...
Для класифікації через `[CLS]` токен, ми можемо проаналізувати attention weights між `[CLS]` та іншими токенами:

$$\alpha_{[CLS], j} = \frac{\exp(\text{score}_{[CLS], j})}{\sum_{k=1}^{n} \exp(\text{score}_{[CLS], k})}$$

де $\text{score}_{[CLS], j} = \frac{\mathbf{q}_{[CLS]} \cdot \mathbf{k}_j}{\sqrt{d_k}}$.

**Інтерпретація:**
- Високий $\alpha_{[CLS], j}$ означає, що токен $j$ сильно впливає на класифікацію
- Низький $\alpha_{[CLS], j}$ означає, що токен $j$ майже не впливає на рішення
...
**Визначення:** Ентропія Шеннона вимірює невизначеність випадкової величини.
...
**Інтерпретація:**
- $H(X) = 0$ — повна визначеність (одна подія з ймовірністю 1)
- $H(X)$ максимальна — максимальна невизначеність (всі події рівноймовірні)
...
Низька ентропія — висока невизначеність класу (один клас домінує).

### Крос-Ентропія (Cross-Entropy)

**Визначення:** Крос-ентропія між двома розподілами $P$ (істинний) та $Q$ (передбачений):
...
де:
- $y_c$ — істинна ймовірність класу $c$ (one-hot encoding: 1 для правильного класу, 0 для інших)
- $\hat{y}_c$ — передбачена ймовірність класу $c$

**Приклад:**

Для логу "Connection refused" з істинним класом Critical:

$$y = [0, 1] \quad \text{(Normal=0, Critical=1)}$$
$$\hat{y} = [0.1, 0.9] \quad \text{(передбачення моделі)}$$

$$H(y, \hat{y}) = -0 \cdot \log 0.1 - 1 \cdot \log 0.9 = -\log 0.9 \approx 0.105$$

Якщо модель помиляється: $\hat{y} = [0.9, 0.1]$

$$H(y, \hat{y}) = -0 \cdot \log 0.9 - 1 \cdot \log 0.1 = -\log 0.1 \approx 2.303$$

**Висновок:** Крос-ентропія більша, коли передбачення далі від істини.
...
$$\hat{c} = \arg\max_{c} P(c | \mathbf{x}) = \arg\max_{c} \log P(c | \mathbf{x})$$

**Еквівалентність мінімізації крос-ентропії:**

Максимізація $\log P(c | \mathbf{x})$ еквівалентна мінімізації $-\log P(c | \mathbf{x})$, що є крос-ентропією для одного прикладу.

**Формалізація:**

Для навчального набору $\{(x_i, y_i)\}_{i=1}^{N}$:

**Максимізація лог-ймовірності (Maximum Likelihood):**

$$\max_{\theta} \sum_{i=1}^{N} \log P(y_i | x_i; \theta)$$

**Еквівалентна мінімізації негативної лог-ймовірності:**

$$\min_{\theta} -\sum_{i=1}^{N} \log P(y_i | x_i; \theta)$$

**Це є сумою крос-ентропій:**

$$\min_{\theta} \sum_{i=1}^{N} H(y_i, \hat{y}_i) = \min_{\theta} -\sum_{i=1}^{N} \sum_{c=1}^{C} y_{i,c} \log \hat{y}_{i,c}$$

де $\hat{y}_{i,c} = P(c | x_i; \theta)$ — передбачення моделі.
...
$$\text{Naive Bayes: } \arg\max_{c} P(c | \mathbf{x}) \propto \arg\max_{c} \log P(\mathbf{x} | c) + \log P(c)$$

$$\text{BERT: } \arg\max_{c} \hat{y}_c = \arg\max_{c} \text{softmax}(\mathbf{W} \mathbf{h}_{[CLS]} + \mathbf{b})_c$$

Обидва підходи мінімізують крос-ентропію, але:
- **Naive Bayes:** Параметри — це ймовірності $P(w | c)$, оцінюються через підрахунок
- **BERT:** Параметри — це ваги нейронної мережі $\theta$, оцінюються через градієнтний спуск

### Крос-Ентропія як Функція Втрат у BERT

**Формалізація:**

Для класифікації з $C$ класами:
...
- $\hat{y}_{i,c}(\theta) = \text{softmax}(\mathbf{W} \mathbf{h}_{[CLS]}^{(i)} + \mathbf{b})_c$ — передбачення моделі
...
**Проблема:** MSE не підходить для класифікації, бо не враховує дискретність класів.

**Крос-ентропія:** Спеціально розроблена для ймовірнісних передбачень.
