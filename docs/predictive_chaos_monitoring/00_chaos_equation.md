...
Нерухома точка **стабільна**, якщо $\lvert f_r'(x^*) \rvert < 1$:

- Для $x_0^* = 0$: $\lvert f_r'(0) \rvert = \lvert r \rvert < 1$ → стабільна при $r < 1$
- Для $x_1^* = 1 - \frac{1}{r}$: $\lvert f_r'(x_1^*) \rvert = \lvert 2 - r \rvert < 1$ → стабільна при $1 < r < 3$
