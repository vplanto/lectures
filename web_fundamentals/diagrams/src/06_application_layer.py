...
with Diagram("Application protocols", show=False, filename="../out/06_application_layer_001"):
    ...
    computer >> Edge(label="Hypertext Transport Protocol HTTP/HTTPS", dir="both") << server
    router >> Edge(color="orange")  >> isp
    isp  >> Edge(color="orange")  >> cloud1
    cloud1 >> Edge(color="red") >> cloud2
    cloud2  >> Edge(color="orange")  >> server    

with Diagram("Application Layer", show=False, filename="../out/06_application_layer_002", graph_attr={"fontsize": "40"}):
    computer = Client("Computer")
    router = Router("Router")
    isp = Server("ISP")
    cloud1 = CloudFront("Internet")
